...
// Se ha eliminado la variable processedButtonIds ya que no es necesaria para la lógica del dueño.
...
export async function handlePaymentProofButton(m, conn, lastSelectedServiceId) {
    if (m.isOwner && m.text) {
        ...
        // La lógica de processedButtonIds se ha eliminado para permitir la re-ejecución del comando del dueño.
        ...
        // Lógica para confirmar la venta y descontar stock
            ...
            try {
                // Regex para extraer el JID del cliente y el serviceId. El serviceId es el resto de la cadena.
                const match = selectedId.match(/^confirm_sale_([^@]+@[^@]+)_(.*)$/);
                if (!match) {
                    await m.reply('Ocurrió un error al procesar el botón de confirmación de venta. Formato inválido.');
                    return false;
                }
                const [, clientJid, serviceId] = match;
