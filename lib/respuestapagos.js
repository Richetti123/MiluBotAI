        ...
        if (selectedId.startsWith('accept_payment_') || selectedId.startsWith('reject_payment_')) {
            ...
            try {
                ...
                if (selectedId.startsWith('accept_payment_')) {
                    ...
                    const lastSelectedService = paymentsData[formattedNumberForAdmin]?.lastSelectedServiceId; // <-- LECTURA CORRECTA DESDE PAGOS.JSON
                    console.log(chalk.magenta(`[DEBUG] lastSelectedServiceId recuperado para ${formattedNumberForAdmin} desde pagos.json: ${lastSelectedService}`));
